...
class TestEvaluator():
    ...
    def __init__(self, model_path, dataset_dir, batch_size=16):
        self.device = 'cuda'
        self.model = load_model(module_class=MyLightningModule, checkpoint_path=Path(model_path), map_location=self.device).to(self.device).eval()
        self.dataset_dir = dataset_dir
        self.transformer_name = "sentence-transformers/LaBSE"
        self.tokenizer: PreTrainedTokenizer = AutoTokenizer.from_pretrained(self.transformer_name)
    ...
    def encode_samples(self, sources_samples, targets_samples):
        ...
        sources_embeds = torch.stack([torch.tensor(g) for s in sources_samples for g in s["sources_embs"]])
        targets_embeds = torch.stack([torch.tensor(g) for s in targets_samples for g in s["targets_embs"]])
    ...
    def evaluate(self, sources, targets):
        ...
        for i in range(math.ceil(len(sources)/self.batch_size)):
            ...
            index = faiss.index_factory(self.sources_emb_matrix.shape[1], "Flat", faiss.METRIC_INNER_PRODUCT)
        ...
        for k,v in self.source_result.items():
            if len(v) > 1:
                ...
                if not (v_indexes == list(range(min(v_indexes), max(v_indexes)+1))):
                    ...
                    source_emb = self.sentence_encoder.encode(source_text_comb, show_progress_bar = False)

                    v_indexes_combs = []
                    for r in range(1, len(v_indexes)+1):
                        v_indexes_combs.extend(list(map(list, combinations(v_indexes, r))))
                    v_indexes_combs = [elem for elem in v_indexes_combs if sorted(elem) == list(range(min(elem), max(elem)+1))]
                    ...
                    for elem in v_indexes_combs:
                        new_elem = elem
                        new_elem.extend([idx for idx in list(range(max(0, min(new_elem) - 3), min(max(new_elem) + 3, len(targets))))])
                    ...
                    targets_embs = self.sentence_encoder.encode(targets_text_combs, show_progress_bar = False)
                    for target_ids_comb, target_emb in zip(true_targets_ids_combs, targets_embs):
                        cs = cosine_similarity(torch.tensor(source_emb), torch.tensor(target_emb))
                        if cs > sim:
                            sim = cs
            ...
            for file_as_source in os.listdir(self.dataset_dir + "/" + book_dir):
                ...
                for file_as_target in os.listdir(self.dataset_dir + "/" + book_dir):
                        if file_as_target.endswith(".jsonl"):
                            continue
                        target_file = os.path.join(self.dataset_dir, book_dir, file_as_target)
                        ...
                        if file_as_source != file_as_target and source_lang != target_lang:
                            ...
                            print(f'CURRENT TEST FILE: {source_lang} -> {target_lang}: {book_name}')
                            ...
                            for i, line in enumerate(source_dataset):
                                s = line.strip()
                                sid = "s" + str(i)
                                source_emb = self.sentence_encoder.encode(s, show_progress_bar = False)
                                sources.append(dict(sources_ids = [sid], sources = [s], sources_embs = [source_emb]))
                            ...
                            for j, line in enumerate(target_dataset):
                                t = line.strip()
                                tid = "t" + str(j)
                                target_emb = self.sentence_encoder.encode(t, show_progress_bar = False)
                                targets.append(dict(targets_ids = [tid], targets = [t], targets_embs = [target_emb]))
                            ...
                            self.write_results_file(book_dir, book_name, source_lang, target_lang)
        
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="TestEvaluator arguments usage: ", add_help=True)
    parser.add_argument("model_path", help="Path to the model checkpoint.")
    parser.add_argument("test_dataset_dir", help="Path to the test dataset to be aligned.")
