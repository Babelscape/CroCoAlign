...
class DatasetGenerator():
    ...
    def generate_dataset_from_xml(self, source_path, target_path, labels_path, slang, tlang):
        ...
        result = jsonlines.open(slang + "-" + tlang + "-" + self.path_leaf(source_path)[0:-4] + ".jsonl", mode='w')
    ...
    def generate_dataset_from_txt(self, source_path, target_path, labels_path, slang, tlang):
        ...
        result = jsonlines.open(slang + "-" + tlang + "-" + self.path_leaf(source_path)[0:-3] + ".jsonl", mode='w')
    ...
    def generate_dataset_from_bleualign(self, source_path, target_path, labels_path, slang, tlang):
        source_file = open(source_path, "r").readlines()
        target_file = open(target_path, "r").readlines()
        labels_file = open(labels_path, "r").readlines()
        ...
        for pair in labels:
            ...
            targets = pair[1]
            row = defaultdict()
            row['sources'] = {'ids': list(sources), 'text': [source_dict[sid] for sid in sources if sid != '']}
            row['targets'] = {'ids': list(targets), 'text': [target_dict[tid] for tid in targets if tid != '']}
            result.write(row)

        result.close()

    def generate_dataset(self, source_path, target_path, labels_path, slang, tlang, format):
        if format == "xml":
            self.generate_dataset_from_xml(source_path, target_path, labels_path, slang, tlang)
        elif format == "bleualign":
            self.generate_dataset_from_bleualign(source_path, target_path, labels_path, slang, tlang)
        else:
            self.generate_dataset_from_txt(source_path, target_path, labels_path, slang, tlang)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="DatasetGenerator usage: ", add_help=True)
    parser.add_argument("source_path", help="Path to the file containing the source file (supported format: .xml).")
    parser.add_argument("target_path", help="Path to the file containing the target file (supported format: .xml).")
    parser.add_argument("labels_path", help="Path to the file containing the ground truth (supported format: opus .tmp).")
    parser.add_argument("--slang", nargs="?", help="Source language.")
    parser.add_argument("--tlang", nargs="?", help="Target language.")
    parser.add_argument("--format", nargs="?", help="Format of the source and target files (.xml, .txt)")
    args = parser.parse_args()
    DatasetGenerator().generate_dataset(args.source_path, args.target_path, args.labels_path, args.slang, args.tlang, args.format)
