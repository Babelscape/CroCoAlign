    ...
    def from_jsonl_to_txt(self, filedir):
        ...
        for filename in os.listdir(filedir):
            ...
            for line in fin:
                ...
                for sid, stext in zip(example["sources"]["ids"], example["sources"]["text"]):
                    source_txt_file.write(stext)
                    source_txt_file.write("\n")
                    # source_txt_id_list.write(sid)
                    # source_txt_id_list.write("\n")
                for tid, ttext in zip(example["targets"]["ids"], example["targets"]["text"]):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="FileConverter (from xml to txt) arguments usage: ", add_help=True)
    parser.add_argument("filedir", help="Path to the directory containing the files to be converted to .txt. Supported formats: xml, jsonl.")
